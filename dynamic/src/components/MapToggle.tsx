...
export default function MapToggle({ properties, viewMode, onViewModeChange: _onViewModeChange }: MapToggleProps) {
  ...
  const propertiesWithCoordinates = properties.filter(p => p.lat && p.lon);
  const centerLat = propertiesWithCoordinates.reduce((sum, p) => sum + (p.lat || 0), 0) / propertiesWithCoordinates.length;
  const centerLon = propertiesWithCoordinates.reduce((sum, p) => sum + (p.lon || 0), 0) / propertiesWithCoordinates.length;
  ...
  return (
    <div className="relative h-[600px] w-full rounded-3xl overflow-hidden border border-gp-ink/10">
      <Suspense fallback={
        ...
        <MapContainer
          center={[centerLat || 13.0827, centerLon || 77.5946]}
          zoom={10}
