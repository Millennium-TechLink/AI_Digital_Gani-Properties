...
export default function PropertiesPage() {
  ...
  const filteredProperties = useMemo(() => {
    let filtered: Property[] = Array.isArray(properties) ? [...properties] : [];
    ...
    if (filters.price) {
      filtered = filtered.filter(p => {
        ...
        const price = parseInt(priceMatch[1]) || 0;
    ...
    if (filters.sort === 'newest') {
      ...
      filtered.sort((a, b) => {
        const aPrice = parseInt(a?.priceLabel?.match(/(\d+)/)?.[1] || '0');
        const bPrice = parseInt(b?.priceLabel?.match(/(\d+)/)?.[1] || '0');
