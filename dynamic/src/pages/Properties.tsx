...
export default function PropertiesPage() {
  ...
  const filteredProperties = useMemo(() => {
    ...
    if (filters.type) {
      // Match by category prefix to support simplified categories
      // e.g., "residential-plots" matches all types starting with "residential-"
      filtered = filtered.filter(p => {
        if (filters.type === 'residential-plots') {
          return p.type.startsWith('residential-');
        } else if (filters.type === 'commercial') {
          return p.type.startsWith('commercial');
        } else if (filters.type === 'retail') {
          return p.type.startsWith('retail');
        } else if (filters.type === 'hospitality') {
          return p.type.startsWith('hospitality');
        } else if (filters.type === 'industrial') {
          return p.type.startsWith('industrial');
        } else if (filters.type === 'farm-plots') {
          return p.type === 'farm-plots' || p.type === 'agricultural-lands';
        } else {
          // Fallback to exact match for any other types
      ...
      <div className="container mx-auto px-4 lg:px-6 py-12">
        ...
        {error && (
          <div className="text-center py-20">
            ...
            <p className="text-gp-ink-muted mb-4">{error}</p>
            <Button onClick={loadProperties} variant="default">
              Try Again
            </Button>
          </div>
        )}
