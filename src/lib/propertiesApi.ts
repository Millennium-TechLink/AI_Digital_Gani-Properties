...
export const propertiesApi = {
  ...
  getAll: async (): Promise<Property[]> => {
    try {
      ...
      // Fallback to local data if API is unreachable (Network Error)
      if (axios.isAxiosError(error) && !error.response) {
        console.warn('API unreachable, falling back to local properties data');
        return localProperties as Property[];
      }
